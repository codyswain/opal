...
- Preview visibility is explicitly toggled and starts closed; selection never changes pane allocation.
...
- [x] Record current npm test, npx tsc --noEmit and npm run lint results before code.
- [x] Add failing component regressions in `filesNavigation.test.tsx` using this
...
- [x] Run `npm test -- src/tests/unit/filesNavigation.test.tsx` and record the
  missing-behavior failures. Add separate cases for single-click selection with
  no tabs or pane changes, explicitly toggled preview, multi-folder selection,
  double-click and Cmd+Down open, Space preview,
  Return rename, breadcrumb navigation, focused-tab activation/close, and Back.
- [x] Make `currentDirectory` the only browse-directory source. Remove
...
- [x] Connect sidebar/breadcrumb/tab/open/return actions to URL intents. Apply
...
- [x] Add and run failing list/gallery restoration tests, then connect react-window
...
- [x] Add failing mutation and async cases, then integrate route remap/removal:
...
- [x] Remove deprecated preview-tab production consumers. Transitional selection
...
- [x] Guard all relevant shortcuts from text input, contenteditable, menus,
  dialogs, independent buttons and handled events. Keep typeahead working.
- [x] Run focused tests for files navigation, route, disk store/path state,
...
- [x] Commit the slice with `feat(files): make browsing and returning preserve context`.
- [x] Independent review checks spec compliance and code quality; fix concrete
  regressions with failing tests before rerunning relevant checks.

## Completion checks

- [x] `npm test` passes, or any reproduced baseline-only failure is documented.
- [x] `npx tsc --noEmit` and `npm run lint` introduce no failures beyond baseline.
- [x] `npm run test:e2e` passes using existing tests; native ABI rebuilds run serially.
- [x] Review a temporary-data Electron launch if available; clearly distinguish
  automated checks from any manual checks that could not be performed.
- [x] Record delivered behavior, tests and deferred roadmap in a verification note.
...
The real Electron walkthrough found that automatically opening the preview pane
on first selection moves gallery tiles between the two native clicks of a
double-click. Preview is therefore an explicit toggle, initially closed. Once
open, its pane remains allocated through selection and directory changes until
the user closes it. This replaces the initial automatic-preview behavior and is
covered alongside the native double-click regressions. Background removal must
also preserve a null openedPath so retained tabs never reopen while browsing.
